...
let com_arity (c : com): int option =
  ...
  | Add  | Sub | Mul | Div | Swap | Concat | And | Or | Equal | Lte -> Some 2
  | Tuple(_) -> None
...
let rec string_of_value v =
  ...
  | VClo(Var(fname), Var(parameter), _prog, _env) -> "Clo (" ^ fname ^ " " ^ parameter ^ ")"
...
let rec zfun (s, l, loc, glob : state) (fname : var) (param : var) (blk : prog) (*(funs : var * var * prog list)*) : (state, eval_err) result =
  Ok(s,
  l,
  (fname, VClo(fname, param, blk, List.append loc glob)) :: loc,
  glob)
  (* match funs with
  | (fname0, fname0, blk0) :: [] -> 
...
and eval (prog : prog) (st : state) : (bool * state, eval_err) result =
  ...
  | Fun(fname, param, blk) -> zfun st fname param blk |> and_then (eval prog)
